...
import { createSocketConnection, formatLastSeen, formatTime } from "../utils/socket";
...
console.log("Selected User:", selectedUser); // Debugging line
  ...
  console.log("Selected User Last Seen:", lastSeen); // Debugging line
  console.log("Selected User Last Seen1:", formatTime(lastSeen)); // Debugging line
    ...
    newSocket.on("messageReceived", (message) => {
      setMessages((prev) => [
        ...prev,
        {
          _id: message._id,
          text: message.text,
          time: formatTime(message.updatedAt || message.createdAt || new Date()),
  ...
  <div className="flex items-center space-x-2">
    <div className="font-semibold text-gray-700">{selectedUser.firstName} {selectedUser.lastName}</div>
    <span
  ...
  <div className="mt-1 text-xs text-gray-500">
    ...
    {messages.map((msg) => (
        <div
          key={msg._id}
          className={`chat ${msg.sender === "self" ? "chat-end" : "chat-start"}`}
        >
